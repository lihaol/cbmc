...
/*******************************************************************\
Function: memory_model_interrupt_pret::operator()
  Inputs:
 Outputs:
 Purpose:
\*******************************************************************/

void memory_model_interrupt_pret::operator()(symex_target_equationt &equation)
{
  print(8, "Adding interrupt constraints");
...
/*******************************************************************\
Function: memory_model_interrupt_pret::nested_isr()
  Inputs:
 Outputs:
 Purpose:
\*******************************************************************/

void memory_model_interrupt_pret::nested_isr(
  symex_target_equationt &equation)
{
     ...
     ++t_it1)
  {
        ...
        ++t_it2)
    {
      ...
      //    ++e_it1)
      for (const auto &e1 : t_it1->second)
      {
        #if 0
        ...
        //    ++e_it2)
        for(const auto &e2 : t_it2->second)
        {
          #if 0
...
/*******************************************************************\
Function: memory_model_interrupt_pret::last
  Inputs:
 Outputs:
 Purpose: compute a before constraint from the last event of the thread
          which the from event is in to the to event
...
exprt memory_model_interrupt_pret::last(const event_it &from, const event_it &to)
{
  const event_listt &events=per_thread_map[from->source.thread_nr];
  exprt::operandst pty_operands;
  pty_operands.reserve(1);
